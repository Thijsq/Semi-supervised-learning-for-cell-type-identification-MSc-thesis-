...
#
# This is the scenario where one class is missing in the test data
#
...
predicted_classes <- predict(supervised_model, as.matrix(test2[,-31]), type = "class")



confusion1 <- table(macosko_labels, predicted_classes)
...
write.csv(supervised_predictions, "lasso_latent_macosko4.csv", row.names = FALSE)
...
predicted_classes <- predict(final_model, as.matrix(test2[,-31]), type = "class")

confusion2 <- table(macosko_labels, predicted_classes)
...
write.csv(final_predictions, "ssl_lasso_latent_macosko4.csv", row.names = FALSE)
confusion1
confusion2
